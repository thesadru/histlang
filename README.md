...
For information on what a sound change is you can check the [Wikipedia page](https://en.wikipedia.org/wiki/Sound_change).
...
Modifiers are defined based on the IPA and match a phoneme and its diacritcs. Some suprasegmental modifiers such as `[+stressed]` or `[-falling tone]` are also supported.

If a group is specified in the output component of a soundchange then its value is based on the matched group with the same name in the input component. If the group in the output uses modifiers than a resulting phoneme will be dynamically generated.
